...
import Portfolio from "./pages/Portfilio/Portfolio";
import Auth from "./pages/Auth/Auth";
import { Route, Routes, useLocation } from "react-router-dom"; // Import useLocation for determining current path
import StockDetails from "./pages/StockDetails/StockDetails";
import Profile from "./pages/Profile/Profile";
import Notfound from "./pages/Notfound/Notfound";
import { useDispatch, useSelector } from "react-redux";
...
function App() {
  ...
  // Determine whether to show the navbar based on current location and user role
  const showNavbar = !auth.user ? false : shouldShowNavbar(location.pathname, routes, auth.user?.role);

  return (
    <>
      {auth.user ? ( // If user is authenticated
        <>
